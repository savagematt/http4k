...
/**
 * The use case for [Changes] is that two processes started off with
...
data class Changes<T>(
    ...
    operator fun <V> invoke(f: Changes<T>.() -> V): V = f(this)
    ...
    /**
     * Drills down using [getter] and gives you another [Changes]
     * object for the value of [getter] in  [reference] vs.
     * the value of [getter] in each of [a] and [b]
     */
    fun <V : Any, R> latest(getter: (T) -> V, f: Changes<V>.() -> R): R =
     ...
     */
    fun <V : Any, R> latest(getter: (T) -> V?, nullValue: V, f: Changes<V>.() -> R): R? {
 ...
 */
fun <T : Any, V> T.changes(b: T, a: T, f: Changes<T>.() -> V) =
    this.changes(b, a).invoke(f)

fun <T : Any> T.changes(b: T, a: T) =
    Changes(this, b, a)

fun <T, V> Changes<T>.join(getter: (T) -> List<V>): List<V> =
    combine(getter, { a, b -> a + b })

fun <T, K, V : Any> Changes<T>.merge(
    getter: (T) -> Map<K, V>,
    nullValue: V,
    f: Changes<V>.() -> V
): Map<K, V> {
...
fun <T, K, V : Any> Changes<T>.mergeNullable(
    getter: (T) -> Map<K, V>?,
    nullValue: V,
    f: Changes<V>.() -> V
): Map<K, V>? {
...
fun <R, V : Any> V.resolve(b: V?, a: V?, f: Changes<V>.() -> R): R? {
    val ref = this
